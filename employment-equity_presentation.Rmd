...
temp <-
  read.csv("ee_dv_salarybins.csv", fileEncoding = "UTF-8-BOM")
...
df <-
  merge(
    ...
    y = {
        bind_rows(
    {temp[, c(1:2)] %>% rename(bins = bins_2021, bin_width = bin_width_2021)},
    {temp[, c(3:4)] %>% rename(bins = bins_2020, bin_width = bin_width_2020)},
    {temp[, c(5:6)] %>% rename(bins = bins_2019, bin_width = bin_width_2019)},
    {temp[, c(7:8)] %>% rename(bins = bins_2018, bin_width = bin_width_2018)},
    {temp[, c(9:10)] %>% rename(bins = bins_2017, bin_width = bin_width_2017)},
    .id = "key"
  ) %>%
  ...
  group_by(Year) %>% 
    mutate(
      ...
      cumsum_non_wm = cumsum(count_non_vm),
      bin_width_cumsum = cumsum(bin_width) # FIXME: it's giving NA for 2017 because it's the first year
    ) %>%
  ungroup() %>%
  pivot_longer(
  ...
  group_by(Year, group) %>%
    mutate(total = ifelse((grepl(pattern = "count_", x = group) & salary_range == "Total"), count_cumsum, NA)) %>%
  fill(total, .direction = "up") %>% 
  ungroup() %>% 
  mutate(half_pop = total/2) %>%
  group_by(Year, group) %>%
  mutate(median_group_ll = median(count_cumsum)) # FIXME

test <- 
  df %>% 
  filter(Year == 2017, group == "count_all", salary_range != "Total")

median(test$count_cumsum) # FIXME: MEDIAN SALARY, I get it now
