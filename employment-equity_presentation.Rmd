...
# Changing plot to function 
plot1_bilingual <- function(language) {
...
salarydataplot1_fr$Subgroup <- factor(salarydataplot1_fr$Subgroup,
                      ...
                      labels = c("Employés ne faisant \n pas partie des minorités visibles", "Employés appartenant \n à une minorité visible",  "Employés en\n  Situation de Handicap", "Employés Indigène", "Employés Noire"))
...
ggplot(plot3_fr[plot3_fr$`Unit of measure` == "DI" &
            plot3_fr$Subgroup %in% c("Employés ne faisant \n pas partie des minorités visibles", "Employés Noire"),]) +
  geom_bar(stat = "identity",
           position = "dodge",
           aes(x = Subgroup,
               y = Value,
  ...
  labs(title = NULL,
       ...
       fill = NULL) + 
  guides(fill = "none", 
         alpha = guide_legend(title = "Èchelle Salariale"))
