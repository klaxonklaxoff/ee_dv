...
# Clear workspace of unneeded df's and functions 2 save mem
...
df <-
  ...
  group_by(Year) %>% # get cumulative frequencies with cumsum()
    mutate(
      ...
      bin_width_cumsum = cumsum(bin_width) 
    ) %>%
  ungroup() %>%
...
est_median <- function(year, ee_group) {
  ...
  median_group_ll <- max(median_df$bin_width_cumsum[median_df[paste("cumsum_", ee_group, sep = "")] < first(median_df[paste("half_pop_", ee_group, sep = "")])[1]], na.rm=TRUE)
  
  # find the cumulative frequncy up the median
  cumsum_uptomedian <- max(median_df[paste("cumsum_", ee_group, sep = "")][median_df[paste("cumsum_", ee_group, sep = "")]< first(median_df[paste("half_pop_", ee_group, sep = "")])[1]], na.rm=TRUE)
  
  # find the frequency of the median group
  freq_mediangroup <- median_df[paste("count_", ee_group, sep = "")][median_df[paste("cumsum_", ee_group, sep = "")] > first(median_df[paste("half_pop_", ee_group, sep = "")])[1]][1]
  
  # find the width of the median group 
  width_mediangroup <- median_df$bin_width[median_df[paste("cumsum_", ee_group, sep = "")] > first(median_df[paste("half_pop_", ee_group, sep = "")])[1]][1]
  
  # calculate median [finally (●'◡'●)]
  median <- median_group_ll + ((first(median_df[paste("half_pop_", ee_group, sep = "")])[1] - cumsum_uptomedian ) / freq_mediangroup) * width_mediangroup  
